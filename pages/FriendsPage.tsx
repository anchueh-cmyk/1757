    ...
    <div className="w-full max-w-2xl animate-in slide-in-from-bottom-5 duration-500 mx-auto px-4">
      ...
      <div className="space-y-4">
        {user.contacts.length > 0 ? user.contacts.map((contact) => (
          <div key={contact.id} className="glass-card rounded-[24px] p-5 flex justify-between items-center group transition-all duration-300 hover:shadow-lg">
            <div className="flex items-center space-x-4">
              <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400 transition-colors group-hover:bg-white">
                {contact.type === 'email' ? (
                  <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
